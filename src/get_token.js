...
try {
    for (let i = 0; i < options.threshold; i++) {
        ...
        if (options.signature) {
            stream.on('end', () => {
                const options = {
                    uri: Addresses[i] + "/dic/" + date_path + "/" + loi_utils.dic_country(provider) + "/" + fetch_anon,
                    body: signed_file,
                    method: 'POST',
                    headers: {
