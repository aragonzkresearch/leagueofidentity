...
commander
    ...
    .option('-s, --signature <value>', 'For digital identity cards (DICs): If this option is used perform an authentication by sending the content of the file <value> signed by the user with his/her own DIC. The file to be signed is generated by first calling this command without this option.')
    .option('-age, --age <value>', 'For digital identity cards (DICs): grant the token only to users born in the year <value> or after if <value> is a positive integer or to users born in the year <-value> or earlier if <value> is a negative integer. A token obtained using this option has to be used with the encrypt, decrypt, sign and verify commands specifying an email of the type \'<value>@id\', where \'id\' can be the SSN of the user or other information (depending on whether the \'--anon\' option is used), and where <value> is either a two character integer specifying a year of birth (e.g. \'91\') or a three characters string consisting of the minus sign (\'-\') followed by a two character integer specifying an year of birth (e.g., \'-91\').')
    .parse(process.argv);
...
try {
    for (let i = 0; i < options.threshold; i++) {
        ...
        if (options.signature) {
            stream.on('end', () => {
                const options = {
                    uri: Addresses[i] + "/dic/" + date_path + "/" + loi_utils.dic_country(provider) + "/" + fetch_anon + "/" + fetch_age,
                    body: signed_file,
                    method: 'POST',
                    headers: {
                ...
                request(options, function(error, response, body) {
                    if (body === "ERROR") {
                        ...
                        pk[Q[i]] = bls.bls12_381.G2.ProjectivePoint.fromHex(body.split('..')[5]);
                        hash[Q[i]] = bls.bls12_381.G1.ProjectivePoint.fromHex(body.split('..')[6]);
                        t--;
                        if (t == 0) Finalize();
                    }
                    return;
        ...
        } else fetch(Addresses[i] + "/" + provider + "/" + group + "/" + date_path + "/" + options.access_token + "/" + fetch_friends + "/" + fetch_anon).then(function(response) {
...
function Finalize() {
    ...
    const id = "LoI.." + provider + ".." + email + ".." + month + ".." + year + ".." + fetch_friends + ".." + fetch_anon;
...
function OutputJSONFile(t) {
    ...
    if (options.json) console.log("JSON document written to file " + options.json + ". Sign this file with your own digital identity card to get the file " + options.json + ".p7m " + "and submit it within " + dic.TIMEOUT_CHALLENGE + "secs to LoI with the command:\nnode get_token.js --threshold t -no_nodes n -l list -P dic.it -s " + options.json + ".p7m [OPTIONS]");
    else console.log("Sign the following JSON document with your own digital identity card to get a file file.p7m " + "and submit it within " + dic.TIMEOUT_CHALLENGE + "secs to LoI with the command:\nnode get_token.js -A null --threshold t -no_nodes n -l list -P dic.it -s file.p7m [OPTIONS]\n");
    LogJSON.log(json);
}
