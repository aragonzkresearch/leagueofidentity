...
Alice logs into her own Google (or Facebook, Twitter, ...) account `alice@gmail.com` from a given `LoI` website and gets what is called an `OAuth 2` `access token`. Alice sends the so obtained `access token` to a sufficiently large set of nodes of `LoI` and can request to these nodes a cryptographich ``token`` for her email address and a given month and such set of nodes, upon verifying that the ``access token`` is valid for the request, send back to Alice a set of `token shares` by means of which Alice can compute the `token` corresponding to her email address and the current month.
...
Alice can use the previously obtained `token` to decrypt the ciphertext computed by Bob and recover the secret message.
...
`LoI` can issue tokens to the holders of valid digital identity cards (`DIC`) and this would allow to create e.g., a `DAO` of the citizens of a given town. As another example, `LoI` could issue tokens only to valid Instagram accounts with more than 1 million of followers thus creating a `DAO of Influencers`.
